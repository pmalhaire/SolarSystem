...
void Sphere::draw() const {
  ...
  glBufferData(GL_ARRAY_BUFFER, getVertexCount() * sizeof(GLfloat),
...
void Sphere::drawLines(const float lineColor[4]) const {
  ...
  glDrawElements(GL_LINES, (unsigned int)lineIndices.size(), GL_UNSIGNED_INT,
                 lineIndices.data());
...
void Sphere::updateRadius() {
  float scale = sqrtf(radius * radius /
                      (vertices[0] * vertices[0] + vertices[1] * vertices[1] +
